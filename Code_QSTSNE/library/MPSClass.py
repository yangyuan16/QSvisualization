...
class MpsOpenBoundaryClass(MpsBasic):
    ...
    * Other inputs:
    ...
    def __init__(self, length, d, chi, spin='half', way='qr', ini_way='r', operators=None,
                 ...
                 is_env_parallel_lmr=True, is_eco_dims=False,load=False):
        ...
        elif ini_way == 'artificial': # initialize MPS by artificialing
            self.mps, self.virtual_dim = T_module.artificial_open_mps(length, d, chi)
        elif ini_way == 'read_mps':
            self.mps, self.virtual_dim = T_module.read_mps(load)
    ...
    def update_tensor_eigs(self, p, index1, index2, coeff1, coeff2, tau, is_real, tol=1e-16):
        ...
        if self.eig_way == 0:
            ...
            h_effect = LinearOp((dim, dim), lambda a: self.update_tensor_eigs_f_handle_optimized(
                a, s, tau))
            # lambda 匿名函数，lambda(a) = f(a, s, tau); f = update_tensor_eigs_f_handle_optimized
            # LinearOp 是一个线性映射。
            #
            #print(h_effect)
...
# ========================================================
    ...
    def observe_bond_energy_from_jxyz(self, pos2, jx, jy, jz, tol=1e-20):
        ...
        for n in range(0, nh):
            if abs(jx) > tol:
                eb[n] += jx * self.observation_s1_s2(([1, 1], pos2[n, :]))
            if abs(jy) > tol:
                eb[n] += jy * np.real(self.observation_s1_s2(([2, 2], pos2[n, :])))
            if abs(jz) > tol:
                eb[n] += jz * self.observation_s1_s2(([3, 3], pos2[n, :]))
        return eb
    ...
    @ staticmethod
    def string_operator(num_op, sz, theta=np.pi):
        op = [sz]
    ...
    def observe_string_order(self, i, j, theta=np.pi):
        tmp = self.mps.copy()
        #tmp = self.copy.deepcopy(self.mps)
        op = self.string_operator(j - i + 1, theta)
...
class MpsFiniteTEBD(MpsOpenBoundaryClass):
    ...
    def __init__(self, length, d, chi, spin='half', ini_way='r', evolve_way='gate'):
        ...
        self.chi = chi
        self.evolve_way = evolve_way  # 'gate' or 'mpo'
...
class MpsInfinite(MpsBasic):
    ...
    def __init__(self, form, d, chi, D, n_tensor=3, n_site=1, spin='half', dmrg_type='mpo',
        ...
        self.bath_op_onsite = np.zeros((chi, chi))
        self.effective_ops = [np.eye(chi) for _ in range(self.operators.__len__())]
        # self.effective_ops = self.operators.copy()
    ...
    def update_bath_onsite(self):
        ...
        self.bath_op_onsite = op.copy()

    @ staticmethod
    def update_by_given_effective_ops(psi, ops, bonds):
        indexes = list()
        ind0 = list(range(-1, -psi.ndim - 1, -1))
        for n in bonds:
            ind0[n] = - ind0[n]
        indexes.append(ind0)
        for n in range(0, ops.__len__()):
            indexes.append([-bonds[n], bonds[n]])
...
def ln_fidelity_per_site_yy(mps1, mps2):
    """
    :param mps1: one MPS
    :param mps2: the other MPS
    :return: ln fidelity per site
    Example:
    >>> f = ln_fidelity_per_site(mps1, mps2)
    """
    length = mps1.__len__()
    v = np.eye(mps1[0].shape[0])
    f_log=0
